...
export function DocumentUploadForm() {
  ...
  return <form className="form-grid" onSubmit={submit}>
    <div className="form-group"><label htmlFor="owner_domain">Evidence domain</label><select className="select" id="owner_domain" name="owner_domain" required defaultValue=""><option value="" disabled>Select domain</option><option value="EXPENSE">Expense</option><option value="IMPORT">Import</option><option value="AUDIT_CASE">Audit case</option><option value="VAT_ADJUSTMENT">VAT adjustment</option><option value="REFUND">Refund</option><option value="BANK_IMPORT">Bank import</option></select></div>
    <div className="form-group"><label htmlFor="owner_resource_id">Owner resource ID</label><input className="field" id="owner_resource_id" name="owner_resource_id" required minLength={2} maxLength={100} placeholder="Resource identifier" /></div>
