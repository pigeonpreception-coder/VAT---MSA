  ...
  return (
    <AppShell active="dashboard" permission="dashboard:read">
      ...
      <section className="metric-grid" aria-label="Key VAT metrics">
        ...
        <article className="metric"><div className="metric-top"><span className="metric-label">Transaction value</span><span className="metric-icon">N$</span></div><div className="metric-value">{formatMoney(snapshot.metrics.total_cents).replace("NAD", "N$")}</div><div className="metric-foot">Gross fiscal value in the pilot ledger</div></article>
        <article className="metric"><div className="metric-top"><span className="metric-label">VAT controlled</span><span className="metric-icon">15</span></div><div className="metric-value">{formatMoney(snapshot.metrics.tax_cents).replace("NAD", "N$")}</div><div className="metric-foot">Output VAT represented by certificates</div></article>
