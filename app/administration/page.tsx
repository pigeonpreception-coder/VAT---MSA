...
export default async function AdministrationPage() {
  ...
  return <AppShell active="administration" permission="administration:read">
    ...
    <section className="metric-grid">
      ...
      <article className="metric"><div className="metric-top"><span className="metric-label">User seats</span><span className="metric-icon">U</span></div><div className="metric-value">{Number(seat?.used_value ?? activeEmployees)} / {seat?.capacity_mode === "UNLIMITED" ? "Unlimited" : seat?.limit_value ?? "—"}</div><div className="metric-foot">{seat?.capacity_mode ?? "NOT CONFIGURED"} · invitations reserve capacity</div></article>
    ...
    {snapshot.capacityExceptions.filter((exception) => exception.status === "OPEN").map((exception) => <div className="alert alert-error" key={String(exception.id)}><strong>Licence capacity exception:</strong> {String(exception.active_users)} active or invited users exceed the activated capacity of {String(exception.licensed_capacity)}. No users were deleted. Deactivate memberships non-destructively or complete an approved upgrade before inviting another employee.</div>)}
    ...
    <section className="panel" id="licensing"><div className="panel-head"><div><h2 className="panel-title">Licence entitlements and usage</h2><div className="panel-meta">No prices configured · expiry is non-destructive</div></div><StatusBadge value={snapshot.license.state} /></div>
      <div className="table-wrap"><table><thead><tr><th>Feature</th><th>Meter</th><th>Usage</th><th>Capacity mode</th><th>Limit</th><th>Entitled</th></tr></thead><tbody>{snapshot.entitlements.map((entitlement) => <tr key={entitlement.feature_key}><td><strong>{entitlement.name}</strong><div className="muted">{entitlement.description}</div></td><td className="mono">{entitlement.metric_key ?? "Unmetered"}</td><td>{Number(entitlement.used_value ?? 0) + Number(entitlement.reserved_value ?? 0)}</td><td><StatusBadge value={entitlement.capacity_mode} /></td><td>{entitlement.capacity_mode === "UNLIMITED" ? "Unlimited" : entitlement.limit_value ?? "Not applicable"}</td><td><StatusBadge value={entitlement.enabled ? "ACTIVE" : "DISABLED"} /></td></tr>)}</tbody></table></div>
