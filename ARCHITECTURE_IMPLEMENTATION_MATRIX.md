...
|---:|---|---|---|---|
| 1 | Identity | Provider registry, immutable provider-subject links, governed environment-labelled authority federation registrations, provisioned-user checks, controlled self-serve posture, durable proofing cases/events, explainable candidate confidence, mismatch evidence and development-only identity fallback | CONTROLLED FOUNDATION | Approved ITAS/enterprise IdP claims and federation contract, MFA, recovery, proofing lifecycle and assurance tests |
...
| 4 | User management | Users, memberships, roles, permissions, grants, authority hierarchy, protected authority-role assignments, quarterly authority reviews and separated privileged portal projections | CONTROLLED FOUNDATION | Enterprise provisioning, invitation/suspension workflows, PAM and independently accepted access certification |
...
| 6 | Customer | Tenant-scoped lifecycle; VAT/TIN/company-registration uniqueness; fail-closed trust profile with explicit provider environment, confidence, tax status and freshness; immutable evidence history; and current-trust enforcement for new quotations/projects | CONTROLLED FOUNDATION | Authoritative NamRA/ITAS/BIPA customer-validation contract, signed provenance, freshness/reconciliation policy and conformance evidence |
| 7 | Supplier | Tenant-scoped lifecycle; VAT/TIN/company-registration uniqueness; fail-closed trust profile with explicit provider environment, confidence, tax status and freshness; immutable evidence history; and current-trust plus active-tax-status enforcement for taxed expenses | CONTROLLED FOUNDATION | Authoritative NamRA/ITAS/BIPA supplier-validation contract, signed provenance, freshness/reconciliation policy and conformance evidence |
...
| 14 | Expense | Tenant-scoped draft capture, integer totals, project/branch links, immutable clean-receipt linkage, database-enforced receipt gating, independent approval/rejection, no-self-approval, immutable decision evidence, audit and outbox | VERIFIED PILOT | Approved production malware scanner, accounting posting/reversal policy and UAT |
...
| 25 | Integration | Provider/connection registry, governed authority-federation lifecycle, webhook metadata/deliveries, sync jobs and health projection; ITAS remains contract-pending | VERIFIED FOUNDATION | Per-provider/federation contracts, credentials, conformance sandbox, mTLS/signatures, key rollover/revocation and DLP review |
...
| 30 | Administration | Separate NamRA Admin and Super Admin policies, scoped authority governance board, hierarchy/federation/role/review projections, independent local-staging decisions, production-activation database gates, service-component health and finance-data exclusion from technical admin | VERIFIED FOUNDATION | Participating-authority appointment, production federation, PAM/JIT controls, five-party activation evidence and independent SoD testing |
...
| Counterparty trust | VAT/TIN/company-registration and legal-name reconciliation; current trust and tax-registration gates; immutable cached snapshots/events; test-only synthetic validation that cannot be relabelled as authority evidence | NamRA/ITAS/BIPA counterparty contract, authoritative status semantics, signed provenance, freshness/reconciliation, outage handling and production conformance remain mandatory |
| Authority governance | Assigned-authority-scoped hierarchy, protected role catalogue and SoD, environment-labelled federation registration, step-up/idempotent onboarding, immutable independent decisions, quarterly review and fail-closed production activation gates | PR-013 participating-authority appointment/federation/conformance/activation evidence plus PR-004 identity/origin and applicable PR-003 ITAS acceptance remain mandatory |
...
- 129 unit/security/policy/database tests passed across 25 test files.
...
- The production build completed and exposed 56 v1 API paths and 70 method/path operations, including the licensed/scoped identity-proofing queue, test-only counterparty verification command and scoped authority-onboarding board/decision commands.
...
- Counterparty-trust migration proof applied `0018`, backfilled existing parties as untrusted pending records, enforced current trust on new customer/supplier transactions, separated synthetic from authority evidence, required active tax status for taxed supplier expenses and proved verification snapshots/events immutable.
- Authority-governance migration proof applied `0019`, registered authority permissions/licence policies, backfilled contract-pending federation and production-blocked onboarding evidence, enforced authority hierarchy and protected-duty SoD, rejected self decisions and direct production activation, and proved decisions/events immutable.
