...
export const selfServeSignupApplications = sqliteTable(
  "self_serve_signup_applications",
  {
    id: text("id").primaryKey(),
    ...
    authorityAttestedAt: text("authority_attested_at").notNull(),
    termsAcceptedAt: text("terms_accepted_at").notNull(),
    privacyNoticeAcceptedAt: text("privacy_notice_accepted_at").notNull(),
    status: text("status").notNull(),
    identityStatus: text("identity_status").notNull(),
    taxpayerVerificationStatus: text("taxpayer_verification_status").notNull(),
    licenceStatus: text("licence_status").notNull(),
    promotedRegistrationApplicationId: text("promoted_registration_application_id").references(() => registrationApplications.id),
    submittedAt: text("submitted_at").notNull(),
  },
  (table) => [
    uniqueIndex("ux_self_serve_signup_reference").on(table.publicReference),
    uniqueIndex("ux_self_serve_signup_contact_key").on(table.contactEmail, table.idempotencyKey),
    index("idx_self_serve_signup_status_submitted").on(table.status, table.submittedAt),
    ...
    check("ck_self_serve_signup_taxpayer_status", sql`${table.taxpayerVerificationStatus} IN ('AWAITING_PROVIDER_CONTRACT','PENDING','VERIFIED','FAILED')`),
    check("ck_self_serve_signup_licence_status", sql`${table.licenceStatus} = 'NOT_ACTIVATED'`),
  ],
);
