...
/**
 * Shared problem+json/response helpers for Module 3's reconciliation
 * matching engine (Phase A). Security fix 2026-08-27
 * (SECURITY_GAP_ASSESSMENT.md item #4): see lib/api/identity.ts's
 * identityProblem for why this is now async and what it records — this
 * route family previously emitted no security events at all.
 */
export async function reconciliationProblem(error: unknown, context: RequestContext): Promise<Response> {
  let status = 500;
