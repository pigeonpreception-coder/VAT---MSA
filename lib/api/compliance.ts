...
export type ComplianceCommand = "OPEN_AUDIT_CASE" | "FILE_DISPUTE" | "REQUEST_REFUND" | "REVIEW_REFUND";
...
export async function handleComplianceCommand(request: Request, permission: string, command: ComplianceCommand, resourceId?: string) {
  ...
  try {
    ...
    return Response.json({ resource: result }, { status: command === "REVIEW_REFUND" ? 200 : 201, headers: { "x-correlation-id": context.correlationId, "cache-control": "no-store" } });
  } catch (error) {
