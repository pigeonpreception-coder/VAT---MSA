...
| Identity, origin and step-up | Production platform headers require `VAT_MSA_IDENTITY_TRUST_MODE=SITES_DISPATCH`. Edge requirements deny direct origin and strip/re-inject identity/assurance headers. The subsequent Issue 1 increment upgrades privileged evidence to a v2 HMAC-SHA256 envelope bound to actor, asserted MFA method, trusted issuer, session, action and origin, with assertion/authentication age limits and single-use digest persistence. | Local tamper, cross-user/session/origin/action, wrong-issuer, expiry, stale/future-time and weak-secret tests pass after the Issue 1 increment; database duplicate digest is rejected. Actual IdP MFA, managed-edge and identity-broker evidence remains externally blocked. |
