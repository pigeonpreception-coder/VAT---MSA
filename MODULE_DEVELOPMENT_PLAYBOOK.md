...
- [x] **Already real, confirmed 2026-08-25:** `CreateBusinessParty`/`UpdateBusinessParty`/`DeactivateBusinessParty` (one shared `business_parties` model tagged `CUSTOMER`/`SUPPLIER` via `party_relationships`, not separate entities) — tenant-scoped, idempotent, duplicate VAT/TIN detection, audit/outbox. **Still missing:** `SearchCustomers`/`SearchSuppliers` (the list endpoint has no filter params) and `VerifySupplier` (does not exist anywhere in the codebase).
