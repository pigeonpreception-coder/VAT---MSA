...
- [x] `AssignReview` (`POST /api/v1/risk-indicators/:id/assignment`, `lib/data/compliance-repository.ts`'s `assignRiskReview`) moves a risk indicator from `OPEN` to `UNDER_REVIEW` once assigned to a validated, active officer — the same existence/active checks `TransitionCase`'s `ASSIGN` action uses. `ApproveAction` (`POST /api/v1/risk-indicators/:id/decision`, `approveRiskAction`) records the human decision — `DISMISS` (a required rationale, no further effect) or `ESCALATE_TO_CASE` — and **is the only path anywhere in this codebase that may create an `AuditCase` from a risk signal**; nothing auto-creates one as a side effect of evaluation, since no evaluation step exists yet (see Phase A below). The new `audit_cases` row is inserted in the same `db.batch()` as the indicator's own status update — atomic, so an indicator can never end up pointing at a case that wasn't actually created. The case's `risk_tier` is taken from the indicator's own `severity` and `opening_reason` from the decision's `rationale`, never independently supplied. `risk_indicators` gained `assigned_officer_id`/`escalated_case_id` columns. Proven by 12 new domain tests (`tests/compliance-domain.test.ts`) and 13 new route-level tests (`tests/routes/module-4-risk-authorisation-gate.test.ts`).
- [ ] **Not built in this phase, deliberately deferred:** `EvaluateRisk` itself (Phase A, below) — the rule engine that would raise new indicators. Today a risk indicator only exists via one seed fixture; this phase built the human gate downstream of wherever an indicator comes from, not the engine that raises them. `GetRestrictedRisk` (a Phase A query) is also not built.
