...
- [x] **Already real, confirmed 2026-08-25:** `recordStockMovement` — real weighted-average-cost recomputation on every write, non-negative-balance enforcement. **Still missing:** Product/Warehouse CRUD (both tables are seed-only — 2 products, 1 warehouse, hardcoded), distinct `Receive`/`Issue`/`Adjust`/`TransferStock` commands (today it's one generic movement-type parameter), a real `TransferStock` atomicity (a transfer needs a caller to issue two separate, unlinked movement calls themselves), and `GetAvailability`/`Valuation` reports (the only read is a raw balance list, no aggregated total).
