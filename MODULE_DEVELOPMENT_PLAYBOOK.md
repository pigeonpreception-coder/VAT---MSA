...
- [x] **Already real, confirmed 2026-08-25:** the fullest domain in this module. `evaluateQuotationLifecycle` is a real state machine; `createQuotation`/`updateQuotation`/`acceptQuotation`/`rejectQuotation`/`expireQuotation`/`convertQuotationToInvoice` all exist with hash-chained `quotation_revisions`. `Convert` genuinely calls Module 2's `submitInvoice` (`lib/data/business-repository.ts` imports it directly from `lib/data/repository.ts`) with the same idempotency key threaded through — no shortcut. **Still missing:** a `Send` status (quotations go straight from create to `ISSUED`, no draft→send transition), `SearchQuotes`, and the `Approval` sub-entity (`quotations.approved_by` exists but no command ever sets it).
