...
- [ ] **Not built in this phase, deliberately deferred:** Phase B's human-authorisation gate (`openAuditCase` remains the only case-creation path — no risk-signal-to-case flow exists yet) and Phase E's segregation-of-duties enforcement (no check yet blocks the same actor from opening, closing, and issuing a finding on one case).
