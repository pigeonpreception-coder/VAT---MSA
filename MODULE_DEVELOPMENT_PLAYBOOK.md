...
**Definition of done:** invoice → certified → VAT-calculated → transaction-posted → correction-capable, fully idempotent under concurrency, fully explainable to a specific rule version, with the unidentified-buyer guarantee under test. Phases A, B, C and E now meet this; only Phase D (no standalone `VATTransaction`/`PostTransaction`/`Reverse`/`Adjust`/`GetTransactionTimeline` — ledger writes remain inline in `submitInvoice`/`cancelInvoice`) remains open.
