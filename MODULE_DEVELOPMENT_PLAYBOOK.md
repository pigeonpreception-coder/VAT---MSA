...
- [x] **Already real, confirmed 2026-08-25:** `createExpense` and `createProject` (with an optional `project_budgets` row, permanently `status='PROPOSED'`). **Confirmed entirely absent, not partially built:** any Expense maker-checker (the `status`/`approved_by`/`approved_at` columns are vestigial — no command ever moves an expense past `DRAFT`), Expense `Category`/`Budget` CRUD (categories are seed-only), `ExpenseReport`, `ApproveBudget` (project budgets are permanently stuck unapproved), `PostCost` (`project_costs` is never written by any application code path, seed-only), and `ProfitabilityReport`.
